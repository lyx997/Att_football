...
def agent(obs, args):
    ...
    with torch.no_grad():
        a_prob, m_prob, _, left_hidden, player_att1 = left_model(state_dict_tensor)
    ...
    return [real_action], int(player_att1)
...
if args.opp:
    ...
    env = footballenv.create_environment(env_name=args.scenario, logdir='dump', write_full_episode_dumps=True, write_video=True, render=args.render, representation='raw', number_of_left_players_agent_controls=0, number_of_right_players_agent_controls=1)
obs = env.reset()
...
while True:
    ...
    while not done:
        if args.opp:
            ...
            action, player_att1 = agent(obs, args)
        obs = env.att_step(action, player_att1)
        left_score = obs[1]
