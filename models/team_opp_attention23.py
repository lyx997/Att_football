...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_cat = nn.Linear(48*3+96*4,arg_dict["lstm_size"])
    ...
    def forward(self, state_dict):
        ...
        player_right_team_embed = torch.cat([player_v1, right_team_att_player_embed], dim=-1) #(1,1,96)
        left_team_right_team_att_embed = torch.add(left_team_v1, -right_team_att_left_embed) #(1,10,96)
        left_team_right_team_embed = torch.cat([left_team_v1, right_team_att_left_embed], dim=-1) #(1,10,96)

        # 2 layer attention ----- Left team embed to Player

        player_q2 = self.fc_q2_attack(player_right_team_att_embed) #(1,1,96)
        player_v2 = self.fc_v2_attack(player_right_team_embed)
        left_team_k2 = self.fc_k2_attack(left_team_right_team_att_embed) #(1,10,96)
        left_team_v2 = self.fc_v2_attack(left_team_right_team_embed)
        ...
        opp_left_team_embed = torch.cat([opp_v1, left_team_att_opp_embed], dim=-1) #(1,1,96)
        right_team_left_team_att_embed = torch.add(right_team_v1, -left_team_att_right_embed) #(1,10,96)
        right_team_left_team_embed = torch.cat([right_team_v1, left_team_att_right_embed], dim=-1) #(1,10,96)

        # 2 layer attention ----- right team embed to opp

        opp_q2 = self.fc_q2_defence(opp_left_team_att_embed) #(1,1,96)
        opp_v2 = self.fc_v2_defence(opp_left_team_embed)
        right_team_k2 = self.fc_k2_defence(right_team_left_team_att_embed) #(1,10,96)
        right_team_v2 = self.fc_v2_defence(right_team_left_team_embed)
