...
################################################################
    ...
    def forward(self, state_dict):
        ...
        player_att1_right = torch.bmm(player_q1, right_team_k1.permute(0,2,1)) / 8 #64
        player_att1_right = F.softmax(player_att1_right, dim=-1)#(1,1,12)

        player_right_att1_embed = torch.bmm(player_att1_right, right_team_v1)#(1,1,64)
        ...
        left_right_att1 = F.softmax(left_team_att1_right, dim=-1)#(1,11,11)

        left_team_right_att1_embed = torch.bmm(left_right_att1, right_team_v1)#(1,11,64)
        ...
        ball_att1_left = torch.bmm(ball_q1, left_team_k1.permute(0,2,1)) / 8 #64
        ball_att1_left = F.softmax(ball_att1_left, dim=-1)#(1,1,12)

        ball_left_att1_embed = torch.bmm(ball_att1_left, left_team_v1)#(1,1,64)
        ...
        right_team_left_att1_embed = torch.bmm(right_left_att1, left_team_v1)#(10,1,64)
        ...
        return prob, prob_m, v, h_out, [player_att1_right.view(horizon, batch, 11), left_right_att1.view(horizon, batch, 11, 11), player_left_att2.view(horizon, batch, 11)], \
