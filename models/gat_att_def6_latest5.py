...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_att1_attack_ws = nn.Linear(64,64, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_attack_ws.weight, gain=1.414)
        self.fc_att1_attack_as = nn.Linear(128,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_attack_as.weight, gain=1.414)
        self.fc_att2_attack_ws = nn.Linear(64,64, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_attack_ws.weight, gain=1.414)
        self.fc_att2_attack_as = nn.Linear(128,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_attack_as.weight, gain=1.414)
        self.conv1d_att1_attack_ws = nn.Conv1d(64, 64, 1, stride=1, bias=False)
        self.conv1d_att1_attack_as = nn.Conv1d(128, 1, 1, stride=1, bias=False)

        self.fc_att1_defence_ws = nn.Linear(64,64, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_defence_ws.weight, gain=1.414)
        self.fc_att1_defence_as = nn.Linear(128,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_defence_as.weight, gain=1.414)
        self.fc_att2_defence_ws = nn.Linear(64,64, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_defence_ws.weight, gain=1.414)
        self.fc_att2_defence_as = nn.Linear(128,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_defence_as.weight, gain=1.414)

        self.conv1d_att1_defence_ws = nn.Conv1d(64, 64, 1, stride=1, bias=False)
        self.conv1d_att1_defence_as = nn.Conv1d(128, 1, 1, stride=1, bias=False)
    ...
    def forward(self, state_dict):
        ...
        right_team_left_ws1 = self.conv1d_att1_defence_ws(right_team_state_embed.permute(0,2,1)).permute(0,2,1)
        right_team_left_ws1_repeat = self.conv1d_att1_defence_ws(right_team_embed_repeat.permute(0,2,1)).permute(0,2,1)
        right_team_left_ws1_repeat = right_team_left_ws1_repeat.reshape(horizon*batch*n_right, n_left+1, -1)

        left_right_ws1 = self.conv1d_att1_defence_ws(left_team_embed_repeat.permute(0,2,1)).permute(0,2,1)
        left_right_ws1 = left_right_ws1.reshape(horizon*batch*n_right, n_left, -1)
        right_team_left_ws1 = right_team_left_ws1.reshape(horizon*batch*n_right, 1, -1)
        ...
        right_team_att1 = F.leaky_relu(self.conv1d_att1_defence_as(right_team_ws1.permute(0,2,1))).permute(0,2,1)
        ...
        left_team_right_ws1 = self.conv1d_att1_attack_ws(left_team_state_embed.permute(0,2,1)).permute(0,2,1)
        left_team_right_ws1_repeat = self.conv1d_att1_attack_ws(left_team_embed_repeat.permute(0,2,1)).permute(0,2,1)
        left_team_right_ws1_repeat = left_team_right_ws1_repeat.reshape(horizon*batch*n_left, n_right+1, -1)#(1*10,12,64)

        right_left_ws1 = self.conv1d_att1_attack_ws(right_team_embed_repeat.permute(0,2,1)).permute(0,2,1)
        right_left_ws1 = right_left_ws1.reshape(horizon*batch*n_left, n_right, -1)#(1*10,11,64)
        left_team_right_ws1 = left_team_right_ws1.reshape(horizon*batch*n_left, 1, -1)
        ...
        left_team_att1 = F.leaky_relu(self.conv1d_att1_attack_as(left_team_ws1.permute(0,2,1))).permute(0,2,1)
