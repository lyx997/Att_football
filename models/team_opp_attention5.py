...
################################################################
        ...
        self.fc_cat = nn.Linear(128+128+128+64*4,arg_dict["lstm_size"])
    ...
    def forward(self, state_dict):
        ...
        player_v2 = self.fc_v2(player_right_att1_embed)#(1,1,64)
        ...
        player_left_att2 = F.softmax(player_left_att2, dim=-1)#(1,1,10)
        ...
        ##-defence_attention---------------------------------------------------------------------------
        # 1 layer attention ----- Right team embed to Ball, Left team
        ball_q1 = self.fc_q1(ball_state)
        ball_k1 = self.fc_k1(ball_state)
        ball_v1 = self.fc_v1(ball_state)#(1,1,64)
        right_team_q1 = self.fc_q1(right_team_state)
        right_team_k1 = self.fc_q1(right_team_state)
        right_team_v1 = self.fc_v1(right_team_state)#(1,10,64)
        left_team_k1 = self.fc_k1(left_team_state)
        left_team_v1 = self.fc_v1(left_team_state)#(1,11,64)
        ...
        right_team_v1 = right_team_v1.view(horizon*batch, n_right, 1, -1)#(1,10,1,64)
        right_team_left_v1 = torch.cat([right_team_v1, left_team_v1_repeat], dim=2)#(1,10,12,64)
        ...
        ball_q2 = self.fc_q2(ball_left_att1_embed)#(1,1,64)
        ball_v2 = self.fc_v2(ball_left_att1_embed)#(1,1,64)
        right_team_k2 = self.fc_k2(right_team_left_att1_embed)#(1,10,64)
        right_team_v2 = self.fc_v2(right_team_left_att1_embed)#(1,10,64)

        ball_right_att2 = torch.bmm(ball_q2, right_team_k2.permute(0,2,1)) / 8
        ball_right_att2 = F.softmax(ball_right_att2, dim=-1)#(1,1,10)

        right_team_att1_left = right_team_att1_left.permute(1,0,2)
        right_left_att2 = torch.bmm(ball_right_att2, right_left_att1)#(1,1,11)

        right_team_att2_ball_embed = torch.bmm(ball_right_att2, right_team_v2)#(1,1,64)
        ...
        ##---------------------------------------------------------------------------------       
        cat = torch.cat([match_sit_embed, player_sit_embed, ball_sit_embed, player_right_att1_embed, left_team_att2_player_embed, 
                        ball_left_att1_embed, right_team_att2_ball_embed], dim=2)
