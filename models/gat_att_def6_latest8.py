...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_player_state = nn.Linear(arg_dict["feature_dims"]["player_state"],48, bias=False)
        #self.fc_player2_state = nn.Linear(48,48)       
        
        self.fc_ball_state = nn.Linear(arg_dict["feature_dims"]["ball_state"],48, bias=False)
        #self.fc_ball2_state = nn.Linear(48,48)
        
        self.fc_left_state = nn.Linear(arg_dict["feature_dims"]["left_team_state"],48, bias=False)
        self.fc_left2_state = nn.Linear(48*11,96)
        self.fc_right_state  = nn.Linear(arg_dict["feature_dims"]["right_team_state"],48, bias=False)
        self.fc_right2_state  = nn.Linear(48*11,96)

        self.fc_att1_attack_ws = nn.Linear(48,48, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_attack_ws.weight, gain=1.414)
        self.fc_att1_attack_as = nn.Linear(96,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_attack_as.weight, gain=1.414)
        self.fc_att2_attack_ws = nn.Linear(48,48, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_attack_ws.weight, gain=1.414)
        self.fc_att2_attack_as = nn.Linear(96,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_attack_as.weight, gain=1.414)

        self.fc_att1_defence_ws = nn.Linear(48,48, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_defence_ws.weight, gain=1.414)
        self.fc_att1_defence_as = nn.Linear(96,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_defence_as.weight, gain=1.414)
        self.fc_att2_defence_ws = nn.Linear(48,48, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_defence_ws.weight, gain=1.414)
        self.fc_att2_defence_as = nn.Linear(96,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_defence_as.weight, gain=1.414)

        self.fc_cat = nn.Linear(48*2+64*2+96*2,arg_dict["lstm_size"])
        ...
        #self.norm_player_state = nn.LayerNorm(48)
        #self.norm_player2_state = nn.LayerNorm(48)
        #self.norm_ball_state = nn.LayerNorm(48)
        #self.norm_ball2_state = nn.LayerNorm(48)
        #self.norm_left_state = nn.LayerNorm(48)
        self.norm_left2_state = nn.LayerNorm(96)
        #self.norm_right_state = nn.LayerNorm(48)
        self.norm_right2_state = nn.LayerNorm(96)
    ...
    def forward(self, state_dict):
        ...
        player_state_embed = self.fc_player_state(player_state)
        ...
        ball_state_embed = self.fc_ball_state(ball_state)
        ...
        left_team_state_embed = self.fc_left_state(left_team_state)  # horizon, batch, n, dim
        ...
        right_team_state_embed = self.fc_right_state(right_team_state)
        ...
        left_team_embed = left_team_state_embed.view(horizon*batch, 1, n_left*dim)
        ...
        right_team_embed = right_team_state_embed.view(horizon*batch, 1, n_right*dim)

        left_team_embed = F.relu(self.norm_left2_state(self.fc_left2_state(left_team_embed))).view(horizon, batch, -1)
        right_team_embed = F.relu(self.norm_right2_state(self.fc_right2_state(right_team_embed))).view(horizon, batch, -1)
        ...
        player_att1 = self.fc_att1_attack_as(player_ws1)
        ...
        player_right_embed = torch.bmm(player_att1.permute(0,2,1), right_player_ws1)
        ...
        ball_att1 = self.fc_att1_defence_as(ball_ws1)
        ...
        ball_left_embed = torch.bmm(ball_att1.permute(0,2,1), left_ball_ws1)
        ...
        right_team_att1 = self.fc_att1_defence_as(right_team_ws1)
        ...
        right_left_embed = torch.bmm(right_team_att1.permute(0,2,1), left_right_ws1)
        ...
        left_team_att1 = self.fc_att1_attack_as(left_team_ws1)
        ...
        left_right_embed = torch.bmm(left_team_att1.permute(0,2,1), right_left_ws1)
        ...
        player_att2 = self.fc_att2_attack_as(player_ws2)
        ...
        ball_att2 = self.fc_att2_defence_as(ball_ws2)
        ...
        left_player_atted_embed = F.elu(torch.bmm(player_att2, left_team_state_embed)).view(horizon, batch, -1)
        #right_player_atted_embed = torch.bmm(player_right_att1, right_team_state_embed).view(horizon, batch, -1)
        right_ball_atted_embed = F.elu(torch.bmm(ball_att2, right_team_state_embed)).view(horizon, batch, -1)
        ...
        cat = torch.cat([player_sit_embed, ball_sit_embed, 
                    left_player_atted_embed, right_ball_atted_embed, left_team_embed, right_team_embed], -1)
