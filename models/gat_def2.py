...
class Model(nn.Module):
    ...
    def forward(self, state_dict):
        ...
        player_ws = self.fc_att_defence_ws(player_state_embed)
        player_ws_repeat = torch.repeat_interleave(player_ws, repeats=n_all, dim=1)
        all_team_ws = self.fc_att_defence_ws(all_team_state_embed)#(1,22,48)
        player_ws = torch.cat([player_ws_repeat, all_team_ws], dim=-1)
        player_att = F.leaky_relu(self.fc_att_defence_as(player_ws))
