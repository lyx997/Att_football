...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_cat = nn.Linear(48*6,arg_dict["lstm_size"])
    ...
    def forward(self, state_dict):
        ...
        all_left_team_embed = torch.cat([player_state_embed, left_team_state_embed], dim=1)
        all_right_team_embed = torch.cat([opp_state_embed, right_team_state_embed], dim=1)
        ...
        right_att_toplayer = right_att_toleft[:,0,:].unsqueeze(1)#(1,1,11)
        right_att_toplayer_embed = torch.bmm(right_att_toplayer, all_right_team_embed)
        ...
        left_player_att_embed = torch.bmm(left_att_, left_team_state_embed) #(1,1,48)
        
        ##---------------------------------------------------------------------------------
        #player_right_att_embed = player_v2.view(horizon, batch, -1)
        left_player_att_embed = left_player_att_embed.view(horizon, batch, -1)
        right_att_toplayer_embed = right_att_toplayer_embed.view(horizon, batch, -1)
        ...
        left_att_toopp = left_att_toright[:,0,:].unsqueeze(1)#(1,1,11)
        left_att_toopp_embed = torch.bmm(left_att_toopp, all_left_team_embed)
        ...
        right_opp_att_embed = torch.bmm(right_att_, right_team_state_embed) #(1,1,48)
        
        ##---------------------------------------------------------------------------------
        #opp_left_att_embed = opp_v2.view(horizon, batch, -1)
        right_opp_att_embed = right_opp_att_embed.view(horizon, batch, -1)
        left_att_toopp_embed = left_att_toopp_embed.view(horizon, batch, -1)

        cat = torch.cat([player_sit_embed, ball_sit_embed, right_att_toplayer_embed, left_player_att_embed, left_att_toopp_embed, right_opp_att_embed], -1)
