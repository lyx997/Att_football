...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_match_situation = nn.Linear(arg_dict['att_feature_dims']['match_situation'],64)
        #self.fc_match2_situation = nn.Linear(64,64)       

        self.fc_player_situation = nn.Linear(arg_dict['att_feature_dims']['player_situation'],64)
        #self.fc_player2_situation = nn.Linear(64,64)       

        self.fc_ball_situation = nn.Linear(arg_dict['att_feature_dims']['ball_situation'],64)
        #self.fc_ball2_situation = nn.Linear(64,64)       

        self.fc_state = nn.Linear(arg_dict["att_feature_dims"]["player_state"],64)
        #self.fc_player2_state = nn.Linear(64,64)       
        
        #self.fc_ball_state = nn.Linear(arg_dict["att_feature_dims"]["ball_state"],64)
        #self.fc_ball2_state = nn.Linear(64,64)
        
        #self.fc_left_state = nn.Linear(arg_dict["att_feature_dims"]["left_team_state"],64)
        #self.fc_left2_state = nn.Linear(64,64)
        #self.fc_right_state  = nn.Linear(arg_dict["att_feature_dims"]["right_team_state"],64)
        #self.fc_right2_state  = nn.Linear(64,64)

        self.fc_att1_attack_ws = nn.Linear(arg_dict["att_feature_dims"]["player_state"],64, bias=False)
        ...
        self.fc_att1_defence_ws = nn.Linear(arg_dict["att_feature_dims"]["player_state"],64, bias=False)
    ...
    def forward(self, state_dict):
        ...
        player_state_embed = F.relu(self.norm_player_state(self.fc_state(player_state)))
        ...
        ball_state_embed = F.relu(self.norm_ball_state(self.fc_state(ball_state)))
        ...
        left_team_state_embed = F.relu(self.norm_left_state(self.fc_state(left_team_state)))  # horizon, batch, n, dim
        ...
        right_team_state_embed = F.relu(self.norm_right_state(self.fc_state(right_team_state)))
        ...
        player_state = player_state.reshape(horizon*batch, 1, -1)
        ball_state = ball_state.reshape(horizon*batch, 1, -1)
        left_team_state = left_team_state.reshape(horizon*batch, n_left, -1)
        right_team_state = right_team_state.reshape(horizon*batch, n_right, -1)
        ...
        right_team_embed_repeat = torch.repeat_interleave(right_team_state, repeats=n_left+1, dim=1)
        left_team_embed_repeat = torch.repeat_interleave(left_team_state, repeats=n_right, dim=0)
        ...
        left_team_embed_repeat = torch.repeat_interleave(left_team_state, repeats=n_right+1, dim=1)
        right_team_embed_repeat = torch.repeat_interleave(right_team_state, repeats=n_left, dim=0)
