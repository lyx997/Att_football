...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_match_situation = nn.Linear(arg_dict['feature_dims']['match_situation'],24)
        #self.fc_match2_situation = nn.Linear(24,24)       

        self.fc_player_situation = nn.Linear(arg_dict['feature_dims']['player_situation'],24)
        #self.fc_player2_situation = nn.Linear(24,24)       

        self.fc_ball_situation = nn.Linear(arg_dict['feature_dims']['ball_situation'],24)
        #self.fc_ball2_situation = nn.Linear(24,24)       

        self.fc_player_state = nn.Linear(arg_dict["feature_dims"]["player_state"],24)
        #self.fc_player2_state = nn.Linear(24,24)       
        
        self.fc_ball_state = nn.Linear(arg_dict["feature_dims"]["ball_state"],24)
        #self.fc_ball2_state = nn.Linear(24,24)
        
        self.fc_left_state = nn.Linear(arg_dict["feature_dims"]["left_team_state"],24)
        #self.fc_left2_state = nn.Linear(24,24)
        self.fc_right_state  = nn.Linear(arg_dict["feature_dims"]["right_team_state"],24)
        #self.fc_right2_state  = nn.Linear(24,24)

        self.fc_att1_player_attack_ws = nn.Linear(24,24, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_attack_ws.weight, gain=1.414)
        self.fc_att1_player_attack_as = nn.Linear(48,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_attack_as.weight, gain=1.414)
        self.fc_att1_left_attack_ws = nn.Linear(24,24, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_attack_ws.weight, gain=1.414)
        self.fc_att1_left_attack_as = nn.Linear(48,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_attack_as.weight, gain=1.414)
        self.fc_att2_attack_ws = nn.Linear(24,24, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_attack_ws.weight, gain=1.414)
        self.fc_att2_attack_as = nn.Linear(48,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_attack_as.weight, gain=1.414)

        self.fc_att1_ball_defence_ws = nn.Linear(24,24, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_defence_ws.weight, gain=1.414)
        self.fc_att1_ball_defence_as = nn.Linear(48,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_defence_as.weight, gain=1.414)
        self.fc_att1_right_defence_ws = nn.Linear(24,24, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_defence_ws.weight, gain=1.414)
        self.fc_att1_right_defence_as = nn.Linear(48,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att1_defence_as.weight, gain=1.414)
        self.fc_att2_defence_ws = nn.Linear(24,24, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_defence_ws.weight, gain=1.414)
        self.fc_att2_defence_as = nn.Linear(48,1, bias=False)
        #nn.init.xavier_uniform_(self.fc_att2_defence_as.weight, gain=1.414)

        self.fc_cat = nn.Linear(24*9,arg_dict["lstm_size"])

        self.norm_player_situation = nn.LayerNorm(24)
        #self.norm_player2_situation = nn.LayerNorm(64)
        self.norm_ball_situation = nn.LayerNorm(24)
        #self.norm_ball2_situation = nn.LayerNorm(24)
        self.norm_match_situation = nn.LayerNorm(24)
        #self.norm_match2_situation = nn.LayerNorm(24)
        
        self.norm_player_state = nn.LayerNorm(24)
        #self.norm_player2_state = nn.LayerNorm(24)
        self.norm_ball_state = nn.LayerNorm(24)
        #self.norm_ball2_state = nn.LayerNorm(24)
        self.norm_left_state = nn.LayerNorm(24)
        #self.norm_left2_state = nn.LayerNorm(24)
        self.norm_right_state = nn.LayerNorm(24)
        #self.norm_right2_state = nn.LayerNorm(24)
    ...
    def forward(self, state_dict):
        ...
        player_right_ws1 = self.fc_att1_player_attack_ws(player_state_embed)
        player_right_ws1_repeat = torch.repeat_interleave(player_right_ws1, repeats=n_right+1, dim=1)
        right_player_ws1 = self.fc_att1_player_attack_ws(right_team_state_embed)#(1,11,64)
        ...
        player_att1 = F.leaky_relu(self.fc_att1_player_attack_as(player_ws1))
        ...
        right_team_left_ws1 = self.fc_att1_right_defence_ws(right_team_state_embed)
        right_team_left_ws1_repeat = self.fc_att1_right_defence_ws(right_team_embed_repeat)
        right_team_left_ws1_repeat = right_team_left_ws1_repeat.view(horizon*batch*n_right, n_left+1, -1)

        left_right_ws1 = self.fc_att1_right_defence_ws(left_team_embed_repeat)
        ...
        right_team_att1 = F.leaky_relu(self.fc_att1_right_defence_as(right_team_ws1))
        ...
        left_team_right_ws1 = self.fc_att1_left_attack_ws(left_team_state_embed)
        left_team_right_ws1_repeat = self.fc_att1_left_attack_ws(left_team_embed_repeat)
        left_team_right_ws1_repeat = left_team_right_ws1_repeat.view(horizon*batch*n_left, n_right+1, -1)#(1*10,12,64)

        right_left_ws1 = self.fc_att1_left_attack_ws(right_team_embed_repeat)
        ...
        left_team_att1 = F.leaky_relu(self.fc_att1_left_attack_as(left_team_ws1))
