...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_state = nn.Linear(arg_dict["feature_dims"]["player_state"], 64)
        self.fc2_state = nn.Linear(64, 64)

        self.fc_att1_ws = nn.Linear(64,64)
        self.fc_att1_as = nn.Linear(128,1)
        self.fc_att2_ws = nn.Linear(128,128)
        self.fc_att2_as = nn.Linear(256,1)

        self.fc_cat = nn.Linear(64+128+128+64*6,arg_dict["lstm_size"])
    ...
    def forward(self, state_dict):
        ...
        player_state_embed = self.fc2_state(self.fc_state(player_state))
        ball_state_embed = self.fc2_state(self.fc_state(ball_state))
        left_team_state_embed = self.fc2_state(self.fc_state(left_team_state))  # horizon, batch, n, dim
        right_team_state_embed = self.fc2_state(self.fc_state(right_team_state))
        ...
        player_right_ws1 = self.fc_att1_ws(player_state_embed)
        player_right_ws1_repeat = torch.repeat_interleave(player_right_ws1, repeats=n_right, dim=1)
        right_player_ws1 = self.fc_att1_ws(right_team_state_embed)#(1,11,64)
        player_ws1 = torch.cat([player_right_ws1_repeat, right_player_ws1], dim=-1)
        player_att1 = F.leaky_relu(self.fc_att1_as(player_ws1))
        ...
        ball_left_ws1 = self.fc_att1_ws(ball_state_embed)
        ball_left_ws1_repeat = torch.repeat_interleave(ball_left_ws1, repeats=n_left, dim=1)
        left_ball_ws1 = self.fc_att1_ws(left_team_state_embed)
        ball_ws1 = torch.cat([ball_left_ws1_repeat, left_ball_ws1], dim=-1)
        ball_att1 = F.leaky_relu(self.fc_att1_as(ball_ws1))
        ball_att1 = F.gumbel_softmax(ball_att1, dim=1, hard=True)
        left_att1_ball_embed = torch.bmm(ball_att1.permute(0,2,1), left_ball_ws1)
        ball_left_embed = torch.cat([ball_left_ws1, left_att1_ball_embed], dim=-1)

        right_team_left_repeat = state_dict["right_repeat"]
        right_team_embed_repeat = torch.repeat_interleave(right_team_state_embed, repeats=right_team_left_repeat, dim=1)
        left_team_embed_repeat = torch.repeat_interleave(left_team_state_embed, repeats=n_right, dim=1)

        right_team_left_ws1 = self.fc_att1_ws(right_team_state_embed)
        right_team_left_ws1_repeat = self.fc_att1_ws(right_team_embed_repeat)
        right_team_left_ws1_repeat = right_team_left_ws1_repeat.view(horizon*batch*n_right, n_left, -1)

        left_right_ws1 = self.fc_att1_ws(left_team_embed_repeat)
        ...
        right_team_att1 = F.leaky_relu(self.fc_att1_as(right_team_ws1))
        right_team_att1 = F.gumbel_softmax(right_team_att1, dim=1, hard=True)

        left_att1_right_embed = torch.bmm(right_team_att1.permute(0,2,1), left_right_ws1)
        left_att1_right_embed = left_att1_right_embed.view(horizon*batch, n_right, -1)
        right_left_embed = torch.cat([right_team_left_ws1, left_att1_right_embed], dim=-1)

        left_team_right_repeat = state_dict["left_repeat"]
        left_team_embed_repeat = torch.repeat_interleave(left_team_state_embed, repeats=left_team_right_repeat, dim=1)
        right_team_embed_repeat = torch.repeat_interleave(right_team_state_embed, repeats=n_left, dim=1)

        left_team_right_ws1 = self.fc_att1_ws(left_team_state_embed)
        left_team_right_ws1_repeat = self.fc_att1_ws(left_team_embed_repeat)
        left_team_right_ws1_repeat = left_team_right_ws1_repeat.view(horizon*batch*n_left, n_right, -1)#(1*10,11,64)

        right_left_ws1 = self.fc_att1_ws(right_team_embed_repeat)
        ...
        left_team_att1 = F.leaky_relu(self.fc_att1_as(left_team_ws1))
        ...
        left_ball_atted_embed = torch.bmm(ball_att1, left_team_state_embed).view(horizon, batch, -1)
