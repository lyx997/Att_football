...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        #self.fc_att1_attack_ws = nn.Linear(arg_dict["feature_dims"]["left_team_state"],arg_dict["feature_dims"]["left_team_state"])
        #self.fc_att1_attack_as = nn.Linear(2*arg_dict["feature_dims"]["left_team_state"],1)
        #self.fc_att2_attack_ws = nn.Linear(arg_dict["feature_dims"]["left_team_state"],arg_dict["feature_dims"]["left_team_state"])
        #self.fc_att2_attack_as = nn.Linear(2*arg_dict["feature_dims"]["left_team_state"],1)

        #self.fc_att1_defence_ws = nn.Linear(arg_dict["feature_dims"]["left_team_state"],arg_dict["feature_dims"]["left_team_state"])
        #self.fc_att1_defence_as = nn.Linear(2*arg_dict["feature_dims"]["left_team_state"],1)
        #self.fc_att2_defence_ws = nn.Linear(arg_dict["feature_dims"]["left_team_state"],arg_dict["feature_dims"]["left_team_state"])
        #self.fc_att2_defence_as = nn.Linear(2*arg_dict["feature_dims"]["left_team_state"],1)

        self.fc_att1_attack_ws = nn.Linear(arg_dict["feature_dims"]["left_team_state"],64)
        self.fc_att1_attack_as = nn.Linear(128,1)
        self.fc_att2_attack_ws = nn.Linear(64,96)
        self.fc_att2_attack_as = nn.Linear(192,1)

        self.fc_att1_defence_ws = nn.Linear(arg_dict["feature_dims"]["left_team_state"],64)
        self.fc_att1_defence_as = nn.Linear(128,1)
        self.fc_att2_defence_ws = nn.Linear(64,96)
        self.fc_att2_defence_as = nn.Linear(192,1)
    ...
    def forward(self, state_dict):
        ...
        player_right_ws1 = self.fc_att1_attack_ws(player_state)
        player_right_ws1_repeat = torch.repeat_interleave(player_right_ws1, repeats=n_right, dim=1)
        right_player_ws1 = self.fc_att1_attack_ws(right_team_state)#(1,11,64)
        ...
        ball_left_ws1 = self.fc_att1_defence_ws(ball_state)
        ball_left_ws1_repeat = torch.repeat_interleave(ball_left_ws1, repeats=n_left, dim=1)
        left_ball_ws1 = self.fc_att1_defence_ws(left_team_state)
        ...
        left_att1_ball_embed = F.elu(torch.bmm(ball_att1.permute(0,2,1), left_ball_ws1))
        ball_left_embed = torch.add(ball_left_ws1, left_att1_ball_embed)

        right_team_left_repeat = torch.ones(11, device=self.device, dtype=torch.long)*10
        right_team_embed_repeat = torch.repeat_interleave(right_team_state, repeats=right_team_left_repeat, dim=1)
        left_team_embed_repeat = torch.repeat_interleave(left_team_state, repeats=n_right, dim=1)

        right_team_left_ws1 = self.fc_att1_defence_ws(right_team_state)
        right_team_left_ws1_repeat = self.fc_att1_defence_ws(right_team_embed_repeat)
        right_team_left_ws1_repeat = right_team_left_ws1_repeat.view(horizon*batch*n_right, n_left, -1)
        ...
        left_att1_right_embed = F.elu(torch.bmm(right_team_att1.permute(0,2,1), left_right_ws1))
        left_att1_right_embed = left_att1_right_embed.view(horizon*batch, n_right, -1)
        right_left_embed = torch.add(right_team_left_ws1, left_att1_right_embed)

        left_team_right_repeat = torch.ones(10, device=self.device, dtype=torch.long)*11
        left_team_embed_repeat = torch.repeat_interleave(left_team_state, repeats=left_team_right_repeat, dim=1)
        right_team_embed_repeat = torch.repeat_interleave(right_team_state, repeats=n_left, dim=1)

        left_team_right_ws1 = self.fc_att1_attack_ws(left_team_state)
        left_team_right_ws1_repeat = self.fc_att1_attack_ws(left_team_embed_repeat)
        left_team_right_ws1_repeat = left_team_right_ws1_repeat.view(horizon*batch*n_left, n_right, -1)#(1*10,11,64)
        ...
        player_att1_ = player_att1_.permute(0,2,1)
        #ball_att1 = ball_att1.permute(0,2,1)
        ball_att1_ = ball_att1_.permute(0,2,1)
        player_att2_ = player_att2_.permute(0,2,1)
        ball_att2_ = ball_att2_.permute(0,2,1)
        #left_team_att1 = left_team_att1.view(horizon*batch, n_left, n_right)
        left_team_att1_ = left_team_att1_.view(horizon*batch, n_left, n_right)
        #right_team_att1 = right_team_att1.view(horizon*batch, n_right, n_left)
        right_team_att1_ = right_team_att1_.view(horizon*batch, n_right, n_left)

        left_player_atted_embed = torch.bmm(player_att2_, left_team_state_embed).view(horizon, batch, -1)
        right_player_atted_embed = torch.bmm(player_att1_, right_team_state_embed).view(horizon, batch, -1)
        right_ball_atted_embed = torch.bmm(ball_att2_, right_team_state_embed).view(horizon, batch, -1)
        left_ball_atted_embed = torch.bmm(ball_att1_, left_team_state_embed).view(horizon, batch, -1)
        ...
        right_left_player_atted_embed = torch.bmm(right_left_player_att1_, right_team_state_embed).view(horizon,batch, -1)

        left_right_ball_att1_ = torch.bmm(ball_att2_, right_team_att1_)
        #left_right_ball_att1 = torch.bmm(ball_att2, right_team_att1)
        left_right_ball_atted_embed = torch.bmm(left_right_ball_att1_, left_team_state_embed).view(horizon, batch, -1)
    ...
    def make_batch(self, data):
        ...
        s_player_sit_batch = np.array(s_player_sit_batch)
        s_ball_sit_batch = np.array(s_ball_sit_batch)
        s_match_sit_batch = np.array(s_match_sit_batch)
        s_player_batch = np.array(s_player_batch)
        s_ball_batch = np.array(s_ball_batch)
        s_left_batch = np.array(s_left_batch)
        s_right_batch = np.array(s_right_batch)
        avail_batch = np.array(avail_batch)
        h1_in_batch = np.array(h1_in_batch)
        h2_in_batch = np.array(h2_in_batch)

        s_player_sit_prime_batch = np.array(s_player_sit_prime_batch)
        s_ball_sit_prime_batch = np.array(s_ball_sit_prime_batch)
        s_match_sit_prime_batch = np.array(s_match_sit_prime_batch)
        s_player_prime_batch = np.array(s_player_prime_batch)
        s_ball_prime_batch = np.array(s_ball_prime_batch)
        s_left_prime_batch = np.array(s_left_prime_batch)
        s_right_prime_batch = np.array(s_right_prime_batch)
        avail_prime_batch = np.array(avail_prime_batch)
        h1_out_batch = np.array(h1_out_batch)
        h2_out_batch = np.array(h2_out_batch)
