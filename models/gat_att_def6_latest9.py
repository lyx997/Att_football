...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.norm_situation = nn.LayerNorm(64)
        ...
        self.norm_state2 = nn.LayerNorm(96)
    ...
    def forward(self, state_dict):
        ...
        ball_state_embed = F.relu(self.fc_player_state(ball_state))
        ...
        left_team_state_embed = F.relu(self.fc_player_state(left_team_state))  # horizon, batch, n, dim
        ...
        right_team_state_embed = F.relu(self.fc_player_state(right_team_state))
        ...
        left_team_embed = F.relu(self.norm_state2(self.fc_left2_state(left_team_embed))).view(horizon, batch, -1)
        right_team_embed = F.relu(self.norm_state2(self.fc_right2_state(right_team_embed))).view(horizon, batch, -1)
        ...
        left_player_atted_embed = self.norm_state(torch.bmm(player_att2, left_team_state_embed).view(horizon, batch, -1))
        #right_player_atted_embed = torch.bmm(player_right_att1, right_team_state_embed).view(horizon, batch, -1)
        right_ball_atted_embed = self.norm_state(torch.bmm(ball_att2, right_team_state_embed).view(horizon, batch, -1))
        ...
        return prob, prob_m, v, h_out, [player_right_att1.view(horizon, batch, 11), left_right_team_att1.view(horizon, batch, 11, 11), player_att2.view(horizon, batch, 11)], \
                [ball_left_att1.view(horizon, batch, 11), right_left_team_att1.view(horizon, batch, 11, 11), ball_att2.view(horizon, batch, 11)]
