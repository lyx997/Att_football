...
class Model(nn.Module):
    ...
    def forward(self, state_dict):
        ...
        player_k0 = self.fc_player_k0(player_state_embed)
        ...
        player_att0_ball = torch.bmm(player_k0, ball_q0.permute(0,2,1))/8
        ...
        team_att0 = torch.concat([player_att0_ball, left_att0_ball], dim=1)
        team_att0 = F.softmax(team_att0, dim=1)
        right_att0_ball = F.softmax(right_att0_ball, dim=1)

        player_att0_ball = team_att0[:,0,:].unsqueeze(-1)
        left_att0_ball = team_att0[:,1:,:]
