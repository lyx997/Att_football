...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_cat = nn.Linear(96*4,arg_dict["lstm_size"])
    ...
    def forward(self, state_dict):
        ...
        left_team_q1 = self.fc_q1(left_team_state_embed) #(1,11,48)
        left_team_v1 = self.fc_v1(left_team_state_embed) #(1,11,48)
        right_team_k1 = self.fc_k1(right_team_state_embed) #(1,10,48)
        right_team_v1 = self.fc_v1(right_team_state_embed) #(1,10,48)
        ball_k1 = self.fc_k1(ball_state_embed) #(1,1,48)
        ball_v1 = self.fc_v1(ball_state_embed) #(1,1,48)
        ...
        ball_q2 = self.fc_q2(ball_left_team_att_embed) #(1,1,96)
        ball_v2 = self.fc_v2(ball_left_team_att_embed)
        right_team_k2 = self.fc_k2(right_team_left_team_att_embed) #(1,10,96)
        right_team_v2 = self.fc_v2(right_team_left_team_att_embed)
        ...
        right_ball_att_embed = torch.bmm(right_att, right_team_v2) #(1,1,96)
        
        ##---------------------------------------------------------------------------------
        ball_left_att_embed = ball_v2.view(horizon, batch, -1)
        right_ball_att_embed = right_ball_att_embed.view(horizon, batch, -1)

        cat = torch.cat([player_right_att_embed, left_player_att_embed, ball_left_att_embed, right_ball_att_embed], -1)
        ...
        return prob, prob_m, v, h_out, [left_att.view(horizon, batch, n_left), right_team_att_toleft_.view(horizon, batch, 11, 11)], [right_att.view(horizon, batch, n_right), left_team_att_toright_.view(horizon, batch, 10, 12)]#[player_att1_right.view(horizon, batch, 11), left_right_att1.view(horizon, batch, 11, 11), player_left_att2.view(horizon, batch, 11)], \
