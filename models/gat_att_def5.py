...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_cat = nn.Linear(64*3+48*3+96,128)
    ...
    def forward(self, state_dict):
        ...
        ball_right_ws1 = self.fc_att1_defence_ws(ball_state_embed)
        ball_right_ws1_repeat = torch.repeat_interleave(ball_right_ws1, repeats=n_right, dim=1)
        right_ball_ws1 = self.fc_att1_defence_ws(right_team_state_embed)
        ball_ws1 = torch.cat([ball_right_ws1_repeat, right_ball_ws1], dim=-1)
        ball_att1 = F.leaky_relu(self.fc_att1_defence_as(ball_ws1))
        ball_att1 = F.gumbel_softmax(ball_att1, dim=1, hard=True)
        right_ball_atted_embed = torch.bmm(ball_att1.permute(0,2,1), right_ball_ws1).view(horizon, batch, -1)
        ...
        left_player_atted_embed = torch.bmm(player_att2, left_player_ws2).view(horizon, batch, -1)

        cat = torch.cat([match_sit_embed, player_sit_embed, ball_sit_embed, 
                    left_closest_state_embed, right_closest_state_embed, right_ball_atted_embed, left_player_atted_embed], -1)
