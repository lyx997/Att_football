...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_cat = nn.Linear(48 + 96*4,arg_dict["lstm_size"])
    ...
    def forward(self, state_dict):
        ...
        #left_closest_state = state_dict["left_team_closest_state"]
        right_team_state = state_dict["right_team_state"]  
        right_closest_state = state_dict["right_team_closest_state"]
        ...
        #left_closest_embed = F.relu(self.norm_state(self.fc_left_state(left_closest_state)))
        right_closest_embed = F.relu(self.norm_state(self.fc_right_state(right_closest_state)))
        ...
        ball_state_embed = ball_state_embed.reshape(horizon*batch, 1, dim)
        ...
        ball_k1 = self.fc_k1_defence(ball_state_embed) #(1,1,48)
        ball_v1 = self.fc_v1_defence(ball_state_embed) #(1,1,48)
        all_right_team_k1 = torch.cat([ball_k1, right_team_k1], dim=1) #(1,11,48)
        ...
        left_team_att_ball_embed = left_team_att_embed[:,0,:].unsqueeze(1) #(1,1,48)
        left_team_att_right_embed = left_team_att_embed[:,1:,:] #(1,10,48)

        ball_left_team_att_embed = torch.cat([ball_v1, left_team_att_ball_embed], dim=-1) #(1,1,96)
        ...
        ball_q2 = self.fc_q2_defence(ball_left_team_att_embed) #(1,1,96)
        ball_v2 = self.fc_v2_defence(ball_left_team_att_embed)
        ...
        right_team_att = torch.bmm(ball_q2, right_team_k2.permute(0,2,1)) #* 100 #(1,1,10)
        right_att_ = F.softmax(right_team_att, dim=-1)
        #right_att = F.gumbel_softmax(right_team_att, dim=-1, hard=True)

        right_ball_att_embed = torch.bmm(right_att_, right_team_v2) #(1,1,96)
        
        ##---------------------------------------------------------------------------------
        ball_left_att_embed = ball_v2.view(horizon, batch, -1)
        right_ball_att_embed = right_ball_att_embed.view(horizon, batch, -1)

        cat = torch.cat([player_right_att_embed, right_closest_embed, left_player_att_embed, ball_left_att_embed, right_ball_att_embed], -1)
    ...
    def make_batch(self, data):
        ...
        s_match_sit_batch, s_player_sit_batch, s_ball_sit_batch, s_player_batch, s_ball_batch, s_left_batch, s_left_closest_batch, s_right_batch, s_right_closest_batch, avail_batch, right_player_dis_batch, right_left_dis_batch, left_ball_dis_batch, left_right_dis_batch = [],[],[],[],[],[],[],[],[],[],[],[],[],[]
        s_match_sit_prime_batch, s_player_sit_prime_batch, s_ball_sit_prime_batch, s_player_prime_batch, s_ball_prime_batch, s_left_prime_batch, s_left_closest_prime_batch,  \
        ...
        for rollout in data:
            s_match_sit_lst, s_player_sit_lst, s_ball_sit_lst, s_player_lst, s_ball_lst, s_left_lst, s_left_closest_lst, s_right_lst, s_right_closest_lst, avail_lst, right_player_dis_lst, right_left_dis_lst, left_ball_dis_lst, left_right_dis_lst =  [], [], [], [], [], [], [], [],[],[],[],[],[],[]
            s_match_sit_prime_lst, s_player_sit_prime_lst, s_ball_sit_prime_lst, s_player_prime_lst, s_ball_prime_lst, s_left_prime_lst, s_left_closest_prime_lst, \
        ...
        s = {
          ...
          "left_team_closest_state": torch.tensor(s_left_closest_batch, dtype=torch.float, device=self.device).permute(1,0,2),
          "right_team_state": torch.tensor(s_right_batch, dtype=torch.float, device=self.device).permute(1,0,2,3),
          "right_team_closest_state": torch.tensor(s_right_closest_batch, dtype=torch.float, device=self.device).permute(1,0,2),
        ...
        s_prime = {
          ...
          "left_team_closest_state": torch.tensor(s_left_closest_prime_batch, dtype=torch.float, device=self.device).permute(1,0,2),
          "right_team_state": torch.tensor(s_right_prime_batch, dtype=torch.float, device=self.device).permute(1,0,2,3),
          "right_team_closest_state": torch.tensor(s_right_closest_prime_batch, dtype=torch.float, device=self.device).permute(1,0,2),
