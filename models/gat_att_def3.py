...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_ball_state = nn.Linear(arg_dict["feature_dims"]["ball_state"],48)
        
        self.fc_left_state = nn.Linear(arg_dict["feature_dims"]["left_team_state"],48)
        self.fc_right_state  = nn.Linear(arg_dict["feature_dims"]["right_team_state"],48)

        #self.fc_state = nn.Linear(arg_dict["feature_dims"]["player_state"], 64)
        #self.fc2_state = nn.Linear(64, 64)

        self.fc_att1_attack_ws = nn.Linear(48,48)
        self.fc_att1_attack_as = nn.Linear(96,1)
        self.fc_att2_attack_ws = nn.Linear(96,96)
        self.fc_att2_attack_as = nn.Linear(192,1)


        self.fc_att1_defence_ws = nn.Linear(48,48)
        self.fc_att1_defence_as = nn.Linear(96,1)
        self.fc_att2_defence_ws = nn.Linear(96,96)
    ...
    def forward(self, state_dict):
        ...
        left_closest_state_embed = F.relu(self.norm_left_state(self.fc_left_state(left_closest_state)))  # horizon, batch, n, dim
        right_team_state_embed = F.relu(self.norm_right_state(self.fc_right_state(right_team_state)))
        right_closest_state_embed = F.relu(self.norm_right_state(self.fc_right_state(right_closest_state)))  # horizon, batch, n, dim
    ...
    def make_batch(self, data):
        ...
        s_match_sit_batch, s_player_sit_batch, s_ball_sit_batch, s_player_batch, s_ball_batch, s_left_batch, s_left_closest_batch, s_right_closest_batch, s_right_batch, avail_batch = [],[],[],[],[],[],[],[],[],[]
        s_match_sit_prime_batch, s_player_sit_prime_batch, s_ball_sit_prime_batch, s_player_prime_batch, s_ball_prime_batch, s_left_closest_prime_batch, s_right_closest_prime_batch, s_left_prime_batch,  \
        ...
        for rollout in data:
            s_match_sit_lst, s_player_sit_lst, s_ball_sit_lst, s_player_lst, s_ball_lst, s_left_closest_lst, s_right_closest_lst, s_left_lst, s_right_lst, avail_lst =  [], [], [], [], [], [], [], [], [], []
            s_match_sit_prime_lst, s_player_sit_prime_lst, s_ball_sit_prime_lst, s_player_prime_lst, s_ball_prime_lst, s_left_closest_prime_lst, s_right_closest_prime_lst, s_left_prime_lst, \
