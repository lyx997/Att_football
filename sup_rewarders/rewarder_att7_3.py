...
def calc_reward(rew, att_rew, prev_obs, obs):
    ...
    if prev_obs:
        ...
        if prev_owned_ball_team == 1 and owned_ball_team != 0:
            ...
            if active_dis_to_ball <= 0.06:
                not_owned_ball_reward = 0
            elif active_dis_to_ball > 0.06:
    ...
    reward = 5.0*win_reward + 5.0*rew + 0.1*yellow_r + 0.001*ball_position_r + 0.1*att_rew + 0.001*change_ball_owned_reward + 0.1*not_owned_ball_reward
