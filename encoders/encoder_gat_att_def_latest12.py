...
def state_to_tensor(state_dict, h_in):
    ...
    state_dict_tensor = {
      ...
      "left_team_closest_state" : left_closest_state,
      "right_team_state" : right_team_state,
      "right_team_closest_state" : right_closest_state,
      "avail" : avail,
      "hidden" : h_in,
...
class FeatureEncoder:
    ...
    def encode(self, obs, seed):
        ...
        opp_num = int(np.argmin(right_team_distance))

        opp_pos_x, opp_pos_y = obs['right_team'][opp_num]
        ...
        match_situation = np.concatenate((steps_situation, score_situation, side)) #13
        ...
        #obs_right_team = np.array(obs['right_team'])
        #right_pass_line = min(np.min(obs['left_team'][1:,0]), np.min(obs_right_team[1:,0]))
        ...
        #    right_team_state[right_team_over_pass_idx[0], :] = 0
