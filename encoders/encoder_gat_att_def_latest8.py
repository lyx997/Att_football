...
class FeatureEncoder:
    ...
    def encode(self, obs):
        ...
        player_state = np.concatenate((obs['left_team'][player_num], player_direction*100, [player_speed*100, 0., player_tired, 1, 0, 0, 1, 0]))
        ball_state = np.concatenate((obs['ball'][:-1], obs['ball_direction'][:-1]*20, [ball_speed*20, ball_distance, 0., 0, 0, 1, 1, 0]))


        obs_right_team = np.array(obs['right_team'])
        right_pass_line = np.min(obs['left_team'][1:,0])
        left_pass_line = np.max(obs_right_team[1:,0])
        ...
        left_team_onehot = np.zeros((10,3))
        left_team_onehot[:,0] = 1
        left_team_state = np.concatenate((obs_left_team*2, obs_left_team_direction*100, left_team_speed*100, \
                                          left_team_distance*2, left_team_tired, left_team_onehot, np.ones([10,1]), np.zeros([10,1])), axis=1)
        if left_team_over_pass_idx[0].size:
            left_team_state[left_team_over_pass_idx[0], -1] = 1
            left_team_state[left_team_over_pass_idx[0], -2] = 0
        ...
        right_team_state = np.concatenate((obs_right_team*2, obs_right_team_direction*100, right_team_speed*100, \
                                           right_team_distance*2, right_team_tired, right_team_onehot, np.ones([11,1]), np.zeros([11,1])), axis=1)
        if right_team_over_pass_idx[0].size:
            right_team_state[right_team_over_pass_idx[0], -1] = 1
            right_team_state[right_team_over_pass_idx[0], -2] = 0
