...
class FeatureEncoder:
    ...
    def encode(self, obs):
        ...
        player_direction = player_direction / (player_speed + 1e-8)
        ...
        player_situation = np.concatenate((avail[2:], obs['left_team'][player_num], player_direction, [player_speed*100],
        ...
        player_state = np.concatenate((obs['left_team'][player_num]*2, player_direction, [player_speed*100, 0., player_tired, 1., 0., 0.]))
        ball_state = np.concatenate((obs['ball'][:-1]*2, ball_direction, [ball_speed*20, ball_distance, 0., 0., 0., 1.]))
        ...
        left_team_distance_to_ball = np.linalg.norm(obs_left_team - obs['ball'][:-1], axis=1, keepdims=True)
        left_team_speed = np.linalg.norm(obs_left_team_direction, axis=1, keepdims=True)
        left_team_direction = obs_left_team_direction / (left_team_speed + 1e-8)
        ...
        left_team_state = np.concatenate((obs_left_team*2, left_team_direction, left_team_speed*100, \
                                         left_team_distance, left_team_tired, left_team_onehot), axis=1)
        
        left_closest_idx = np.argmin(left_team_distance)
        left_closest_onehot = np.zeros(11)
        ...
        right_team_direction = obs_right_team_direction / (right_team_speed + 1e-8)
        right_team_tired = np.array(obs['right_team_tired_factor']).reshape(-1,1)
        right_team_onehot = np.zeros((11,3))
        right_team_onehot[:,1] = 1.
        right_team_state = np.concatenate((obs_right_team*2, right_team_direction, right_team_speed*100, \
                                          right_team_distance, right_team_tired, right_team_onehot), axis=1)
        ...
        for i in range(11):
            ...
            left_team_distance_to_right.append(np.linalg.norm(obs_left_team - obs['right_team'][i], axis=1, keepdims=True))
        ...
        left_repeat = np.ones(11)*11
        right_repeat = np.ones(11)*11
        ...
        state_dict = {'match_situation':match_situation,
                      ...
                      'left_team_state':left_team_state,
                      'right_team_state':right_team_state,
                      'right_team_distance_to_player':right_team_distance,
                      'left_team_distance_to_ball':left_team_distance_to_ball,
