...
class Algo():
    ...
    def train(self, model, data):
        ...
        for mini_batch in data:
            ...
            right_player_dis = s["right_player_dis"]
            right_player_dis_att = F.softmax(20 * (1 - right_player_dis), dim=-1)
            right_left_dis = s["right_left_dis"]
            right_left_dis_att = F.softmax(20 * (1 - right_left_dis), dim=-1)
            left_ball_dis = s["left_ball_dis"]
            left_ball_dis_att = F.softmax(20 * (1 - left_ball_dis), dim=-1)
        ...
        for i in range(self.K_epoch):
            for mini_batch in data_with_adv:
                ...
                dis_att_loss_lst.append(dis_att_loss.item())
                att_entropy_lst.append(att_entropy.mean().item())
        ...
        return np.mean(tot_loss_lst), np.mean(pi_loss_lst), np.mean(v_loss_lst), np.mean(entropy_lst), np.mean(move_entropy_lst), np.mean(dis_att_loss_lst), np.mean(att_entropy_lst)
