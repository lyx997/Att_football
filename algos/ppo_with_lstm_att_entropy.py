    ...
    def train(self, model, data):
        ...
        for i in range(self.K_epoch):
            for mini_batch in data_with_adv:
                ...
                pi, pi_move, v, _, left_player_att, _ = model(s)
                pi_prime, pi_m_prime, v_prime, _, _, _= model(s_prime)

                player_att_entropy = - torch.log(left_player_att)

                att_entropy = player_att_entropy 
        ...
        return np.mean(tot_loss_lst), np.mean(pi_loss_lst), np.mean(v_loss_lst), np.mean(entropy_lst), np.mean(move_entropy_lst), np.mean(att_entropy_lst)
