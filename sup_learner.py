...
def seperate_write_loss(i, writer, optimization_step, tot_loss_lst, att_loss_lst, att_entropy_lst):

    if i == 0:
        writer.add_scalar('train/off_tot_loss', np.mean(tot_loss_lst), optimization_step)
        writer.add_scalar('train/off_att_loss', np.mean(att_loss_lst), optimization_step)
        writer.add_scalar('train/off_att_entropy', np.mean(att_entropy_lst), optimization_step)

    else:
        writer.add_scalar('train/def_tot_loss', np.mean(tot_loss_lst), optimization_step)
        writer.add_scalar('train/def_att_loss', np.mean(att_loss_lst), optimization_step)
        writer.add_scalar('train/def_att_entropy', np.mean(att_entropy_lst), optimization_step)

def seperate_write_summary(idx,writer, arg_dict, summary_queue, optimization_step, tot_loss_lst, att_loss_lst, att_entropy_lst):
    off_accuracy, def_accuracy = [], []
    for i in range(arg_dict["summary_game_window"]):
        game_data = summary_queue.get()
        off_acc, def_acc = game_data
        off_accuracy.append(off_acc)
        def_accuracy.append(def_acc)

    if idx == 0:
        writer.add_scalar('game/off_accuracy', float(np.mean(off_accuracy)), optimization_step)
        writer.add_scalar('train/off_step', float(optimization_step), optimization_step)
        writer.add_scalar('train/off_tot_loss', np.mean(tot_loss_lst), optimization_step)
        writer.add_scalar('train/off_att_loss', np.mean(att_loss_lst), optimization_step)
        writer.add_scalar('train/off_att_entropy', np.mean(att_entropy_lst), optimization_step)
    else:
        writer.add_scalar('game/def_accuracy', float(np.mean(def_accuracy)), optimization_step)
        writer.add_scalar('train/def_step', float(optimization_step), optimization_step)
        writer.add_scalar('train/def_tot_loss', np.mean(tot_loss_lst), optimization_step)
        writer.add_scalar('train/def_att_loss', np.mean(att_loss_lst), optimization_step)
        writer.add_scalar('train/def_att_entropy', np.mean(att_entropy_lst), optimization_step)
...
def write_summary(writer, arg_dict, summary_queue, n_game, loss_lst, pi_loss_lst, v_loss_lst, \
                  entropy_lst, move_entropy_lst, dis_att_loss_lst, att_entropy_lst, optimization_step, self_play_board, win_evaluation, score_evaluation, good_pass_evaluation, model):
    ...
    writer.add_scalar('train/att_entropy', np.mean(att_entropy_lst), n_game)
    writer.add_scalar('train/dis_att_loss', np.mean(dis_att_loss_lst), n_game)
...
def seperate_save_model(i, model, arg_dict, optimization_step, last_saved_step):
    if optimization_step >= last_saved_step + arg_dict["model_save_interval"]:
        ...
        if i == 0:
            path = arg_dict["log_dir_off"]+"/model_off_"+str(optimization_step)+".tar"
        else:
...
def seperate_learner(i, center_model, queue, signal_queue, summary_queue, arg_dict, writer):
    ...
    if i==0:
        imported_model = importlib.import_module("sup_models." + arg_dict["model_off"])
    else:
        imported_model = importlib.import_module("sup_models." + arg_dict["model_def"])
    ...
    while True:
        if queue.qsize() > arg_dict["batch_size"]*arg_dict["buffer_size"]:
            ...
            if i == 0:
                print("Offence model: step :", optimization_step, "loss", tot_loss, "data_q", queue.qsize(), "summary_q", summary_queue.qsize())
            else:
                print("Defence model: step :", optimization_step, "loss", tot_loss, "data_q", queue.qsize(), "summary_q", summary_queue.qsize())
            
            tot_loss_lst.append(tot_loss)
            att_loss_lst.append(att_loss)
...
def learner(center_model, queue, signal_queue, summary_queue, arg_dict):
    ...
    last_saved_step = optimization_step
    n_game = 0
    loss_lst, pi_loss_lst, v_loss_lst, entropy_lst, move_entropy_lst, dis_att_loss_lst, att_entropy_lst = [], [], [], [], [], [], []
    self_play_board = {}
    ...
    while True:
        if queue.qsize() > arg_dict["batch_size"]*arg_dict["buffer_size"]:
            ...
            signal_queue.put(1)
            data = get_data(queue, arg_dict, model)
            loss, pi_loss, v_loss, entropy, move_entropy, dis_att_loss, att_entropy = algo.train(model, data)
            optimization_step += arg_dict["batch_size"]*arg_dict["buffer_size"]*arg_dict["k_epoch"]
            ...
            dis_att_loss_lst.append(dis_att_loss)
            att_entropy_lst.append(att_entropy)
            center_model.load_state_dict(model.state_dict())
            ...
            if summary_queue.qsize() >= arg_dict["summary_game_window"]:
                win_evaluation, score_evaluation, good_pass_evaluation = write_summary(writer, arg_dict, summary_queue, n_game, loss_lst, pi_loss_lst, 
                                                                 v_loss_lst, entropy_lst, move_entropy_lst, dis_att_loss, att_entropy, optimization_step, 
