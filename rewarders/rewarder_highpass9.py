...
def calc_reward(rew, prev_obs, obs, player_att_idx, action):
    ...
    if  owned_ball_team_prev == 0 and owned_ball_team != 1:
        ...
        if right_team_closest_distance > 0.03:
        ...
        if left_team_closest_distance > 0.02:
            empty_pass_reward = -1.0
        elif left_team_closest_distance < 0.01:
            empty_pass_reward = 1.0



            
    reward = 5.0*win_reward + 5.0*rew + 0.003*ball_position_r + 0.3*yellow_r  + 0.1*change_ball_owned_reward + 0.003*empty_pass_reward
