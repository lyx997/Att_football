...
def calc_reward(rew, prev_obs, obs, prev_most_att_idx, prev_most_att):
    ...
    if prev_obs and prev_most_att_idx:
        ...
        #elif owned_ball_team == 0 and prev_owned_ball_team == 0 and prev_owned_ball_player != owned_ball_player :
        #    change_ball_owned_reward = 1.0

        prev_active = prev_obs['active']
        prev_active_pos_x = obs['left_team'][prev_active][0]
        active_pos_x = obs['left_team'][active][0]
        if  prev_owned_ball_team == 0 and owned_ball_team == 0 and active_pos_x >= 0 and prev_owned_ball_player != owned_ball_player and active in prev_most_att_idx and prev_most_att > 0.3:
            ...
            elif right_team_closest_distance - prev_right_team_closest_distance > 0:
    ...
    reward = 5.0*win_reward + 5.0*rew + 0.003 * ball_position_r + 0.3*yellow_r  + 0.3*change_ball_owned_reward + 0.3*safe_pass_reward 
