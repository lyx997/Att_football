...
def calc_reward(rew, prev_obs, obs, player_att_idx, action):
    ...
    #elif owned_ball_team == 0 and owned_ball_team_prev == -1 and active != 0:
    ...
    if  owned_ball_team_prev == 0 and owned_ball_team == -1 and active_pos_x > 0:
        ...
        prev_right_team_distance = np.linalg.norm(obs_right_team - obs['left_team'][prev_active], axis=1, keepdims=True)
        prev_right_team_closest_distance = np.min(prev_right_team_distance)

        if right_team_closest_distance - prev_right_team_closest_distance > 0.1:
            safe_pass_reward = 3.0
        elif right_team_closest_distance - prev_right_team_closest_distance > 0.04:
    ...
    reward = 5.0*win_reward + 5.0*rew + 0.003 * ball_position_r + 0.3*yellow_r  + 0.1*change_ball_owned_reward #+ 0.1*safe_pass_reward 
