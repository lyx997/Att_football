...
def calc_reward(rew, prev_obs, obs, most_team_att):
    ...
    if most_team_att:
        if obs['active'] == int(most_team_att[0]):
    ...
    reward = 5.0*win_reward + 5.0*rew + 0.003*ball_position_r + 0.3*yellow_r + 0.1*change_ball_owned_reward + 0.1*attention_reward + 0.1*empty_score_reward
