...
def calc_reward(rew, prev_obs, obs, player_att_idx, action):
    ...
    if  owned_ball_team_prev == 0 and owned_ball_team == -1 and active != 0 and ball_position_r > 0:
        ...
        if right_team_closest_distance > 0.1:
            empty_pass_reward = 1.0

    ball_close_reward = 0
    if owned_ball_team_prev != 0:
        obs_player_team = np.array([obs["left_team"][active]])
        #obs_left_team = np.array(obs["left_team"])
        obs_ball = np.array(obs["ball"][:-1])
        left_team_distance = np.linalg.norm(obs_ball - obs_player_team, axis=1, keepdims=True)
        #left_team_closest_distance = np.min(left_team_distance)

        if left_team_distance < 0.01:
            ball_close_reward = 3.0
        elif left_team_distance < 0.03:
            ball_close_reward = 1.0

    reward = 5.0*win_reward + 5.0*rew  + 0.3*yellow_r + 0.003*ball_position_r  + 0.1*change_ball_owned_reward + 0.1*empty_pass_reward + 0.01*ball_close_reward
