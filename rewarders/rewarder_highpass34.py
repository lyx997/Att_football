...
def calc_reward(rew, prev_obs, obs, prev_most_att_idx, prev_most_att, highpass, prev_opp_most_att_idx, prev_opp_most_att):
    ...
    if prev_most_att_idx:
        if active in prev_most_att_idx and prev_most_att > 0.3:
           #good_pass_counts = 1
           attention_reward = float(np.exp(prev_most_att - 0.3))
    ...
    reward = 5.0*win_reward + 10.0*rew + 0.001 * ball_position_r + 0.3*yellow_r + 0.1*safe_pass_reward + 0.05*change_ball_owned_reward + 0.01*attention_reward 
