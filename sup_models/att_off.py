...
class Model(nn.Module):
    def __init__(self, arg_dict, device=None):
        ...
        self.fc_ball_state = nn.Linear(arg_dict["off_feature_dims"]["player_state"],48)
        self.fc_left_state = nn.Linear(arg_dict["off_feature_dims"]["player_state"],48)
        self.fc_right_state = nn.Linear(arg_dict["off_feature_dims"]["player_state"],48)
    ...
    def forward(self, state_dict):
        ...
        ball_q1 = self.fc_q1_offence(ball_state_embed) # 1,1,48
        ball_v1 = self.fc_v1_offence(ball_state_embed) # 1,1,48
        ...
        ball_att_to_right = torch.bmm(ball_q1, right_team_k1.permute(0,2,1)) # 1,1,11
        ball_att_to_right_ = F.softmax(ball_att_to_right, dim=-1)
        ball_att_right_team_embed = torch.cat([ball_v1, torch.bmm(ball_att_to_right_, right_team_v1)], dim=-1) # 1,1,96
        ...
        ball_q2 = self.fc_q2_offence(ball_att_right_team_embed) #1,1,96
        left_team_k2 = self.fc_k2_offence(left_att_right_team_embed)#1,11,96

        ball_att_to_left = torch.bmm(ball_q2, left_team_k2.permute(0,2,1)) #1,1,11
        ball_att_to_left_ = F.softmax(ball_att_to_left, dim=-1)

        return ball_att_to_left_.squeeze(0), left_att_to_right_.squeeze(0)

    def make_batch(self, data):
            # data = [tr1, tr2, ..., tr10] * batch_size
            s_ball_batch, s_left_batch, s_right_batch, label_left_att_batch, right_left_dis_batch = [],[],[],[],[]
            ...
            s = {
              ...
              "label_player_att": torch.tensor(label_left_att_batch, dtype=torch.float, device=self.device),
              "label_opp_att": torch.tensor(right_left_dis_batch, dtype=torch.float, device=self.device).squeeze(-1),
            }
